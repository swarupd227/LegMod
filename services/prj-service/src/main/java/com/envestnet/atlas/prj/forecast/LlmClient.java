...
/**
 * Thin client over the Atlas LLM Gateway, scoped to prj-service's
 * needs (currently only the Migration Forecast call). Mirrors the
 * shape of {@code arch-service}'s {@code LlmClient} so the cost
 * attribution + audit plumbing on the gateway side sees a consistent
 * request envelope across services.
 */
@Component
